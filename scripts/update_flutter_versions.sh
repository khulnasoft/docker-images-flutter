...
# and edits the .khulnasoft.yml file with those versions

releases_json=$(curl -s https://storage.googleapis.com/flutter_infra_release/releases/releases_linux.json)

# This function edits the .khulnasoft.yml file with the given flutter version for the given docker tag
edit_khulnasoft_file_for_tag() {
    khulnasoft_file=".khulnasoft.yml"
    docker_tag=$1
    version=$2

    # env for yq
    docker_tag=$docker_tag version=$version \
        yq -i '(.docker_builder.env.matrix[] | select(.DOCKER_TAG == env(docker_tag)) | .FLUTTER_VERSION) = env(version)' $khulnasoft_file
}
...
edit_khulnasoft_file_for_tag "stable" "$stable_version"
edit_khulnasoft_file_for_tag "latest" "$stable_version"
edit_khulnasoft_file_for_tag "beta" "$beta_version"
